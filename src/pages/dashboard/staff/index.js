...
function Enrolment({ auth }) {
  ...
  const searchOrgs = async (page, query) => {
    setDisabled(true);
    try {
      const response = await InsurerService.searchOrganization(
        JSON.parse(localStorage.getItem("plateaumed_hr_user")).id,
        page,
        perPage,
        query
  ...
  return (
    <>
      <main className="bg-[#EDF0F8] h-screen">
        ...
        <Success
          visible={openSuccess}
          title="Staff list submitted successfully!"
          description="Your staff list has been submitted to Smathealth Medicare Limited"
        ...
        <section className="m-[32px]">
          <div className="flex justify-between items-center mb-8">
            <h5 className="text-[#051438] text-[18px] font-semibold">
              Staff management
            </h5>

            {!disabled && staffs?.staff_members?.length > 0 && (
              <div className="bg-[#DFE2E9] py-[8px] px-[16px] flex gap-7 justify-between items-center rounded-[10px]">
                <div className="flex flex-col  items-center">
                  <span className="text-[#051438] text-[16px] font-semibold">
                    {staffs?.staff_members.length}
                  </span>
                  <span className="text-[#677597] text-[14px] font-semibold">
                    Total staff enrolled
                  </span>
                </div>
                <div className="flex flex-col  items-center">
                  <span className="text-[#051438] text-[16px] font-semibold">
                    {staffs?.incomplete_setup}
                  </span>
                  <span className="text-[#677597] text-[14px] font-semibold">
                    Enrollees with info completed
                  </span>
                </div>

                <div className="flex flex-col  items-center">
                  <span className="text-[#051438] text-[16px] font-semibold">
                    {staffs?.total_dependents}
                  </span>
                  <span className="text-[#677597] text-[14px] font-semibold">
                    Total dependants
                  </span>
            ...
            <div className="flex items-center gap-8">
              <div className="flex gap-3 items-center bg-white border border-[#DFE2E9] rounded-[10px]  pr-[20px]">
                <input
                  ...
                  id="search"
                  name="search"
                  className="w-full py-[10px] pl-[20px] rounded-l-[10px]"
          ...
          {!disabled && staffs?.staff_members?.length == 0 && (
            <div className="h-[540px] flex justify-center items-center overflow-scroll">
              <div className="bg-white rounded-[10px] border border-[#DFE2E9] p-[16px] flex flex-col justify-center items-center h-[360px] w-[640px]">
                <Image
                  ...
                  className="mb-8"
                />
                <p className="text-[#677597] text-[16px] mb-2">
                  No data recorded
                </p>
                <p className="text-[#051438] text-[16px] font-medium">
                  Click on the Upload staff details button to get started
                </p>
              </div>
            </div>
          )}

          {!disabled && staffs == null && (
              ...
              <div>
                <div className="flex gap-6 items-center">
                  <div className="flex gap-2 items-center">
                    ...
                    <span className="text-[#677597] text-[16px] font-medium">
                      {totalPages * perPage}
                    </span>
                  </div>
                  ...
                  <div className="flex gap-3 items-center">
                    <button
                      className={`flex justify-center items-center bg-white disabled:bg-[#E6E8EB] border border-[#DFE2E9] rounded-[4px] h-[32px] w-[32px] ${"cursor-not-allowed"}`}
                    ...
                    <button
                      className={`flex justify-center items-center bg-white disabled:bg-[#E6E8EB] border border-[#DFE2E9] rounded-[4px] h-[32px] w-[32px] ${
                ...
                <div>
                  ...
                  <div className="">
                    <EnrolleeRow
                      enrollees={staffs?.staff_members}
                      openPlan={openEnrollee}
                    />
                  </div>
                </div>
              </div>
